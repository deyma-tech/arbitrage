...
use crate::{
    packet::{
        Meta as ProtoMeta, Packet as ProtoPacket, PacketBatch as ProtoPacketBatch, PacketFlags as ProtoPacketFlags,
    },
    shared::Socket,
};
...
pub fn packet_batches_to_proto_packets(batches: &[PacketBatch]) -> impl Iterator<Item = ProtoPacket> + '_ {
    batches.iter().flat_map(|b| b.iter().filter_map(packet_to_proto_packet))
}
...
pub fn proto_packet_batch_to_packets(packet_batch: ProtoPacketBatch) -> impl Iterator<Item = Packet> {
    packet_batch
        .packets
        .into_iter()
