...
use jito_protos::{
    ...
    bundle::{
        bundle_result::Result as BundleResultType, rejected::Reason, Accepted, Bundle, BundleResult, InternalError,
...
use tonic::{codegen::InterceptedService, transport, transport::Channel, Response, Status, Streaming};
...
pub async fn get_searcher_client(
    ...
    let auth_channel = create_grpc_channel(block_engine_url, cert_path).await?;
    let client_interceptor =
        ClientInterceptor::new(AuthServiceClient::new(auth_channel), auth_keypair, Role::Searcher).await?;

    let searcher_channel = create_grpc_channel(block_engine_url, cert_path).await?;
    let searcher_client = SearcherServiceClient::with_interceptor(searcher_channel, client_interceptor);
    Ok(searcher_client)
}

pub async fn get_searcher_client_without_interceptor(
    block_engine_url: &str,
    cert_path: &str,
...
pub async fn send_bundle_with_confirmation(
    ...
    while let Ok(Some(Ok(results))) =
        timeout(Duration::from_millis(time_left), bundle_results_subscription.next()).await
    {
