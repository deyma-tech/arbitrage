...
use jito_protos::auth::{
    auth_service_client::AuthServiceClient, GenerateAuthChallengeRequest, GenerateAuthTokensRequest,
...
impl ClientInterceptor {
    pub async fn new(
        ...
        let (access_token, refresh_token) = Self::auth(&mut auth_service_client, keypair, role).await?;
    ...
    fn spawn_token_refresh_thread(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let refresh_token_ttl = SystemTime::try_from(refresh_token.expires_at_utc.as_ref().unwrap().clone())
                    .unwrap()
                    .duration_since(SystemTime::now())
                    .unwrap_or_else(|_| Duration::from_secs(0));
