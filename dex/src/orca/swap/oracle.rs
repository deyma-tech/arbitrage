...
use crate::orca::swap::TICK_ARRAY_SIZE;
...
impl AdaptiveFeeConstants {
    ...
    pub fn validate_constants(
        ...
        if tick_group_size == 0 || tick_group_size > tick_spacing || !tick_spacing.is_multiple_of(tick_group_size) {
            return false;
        }
...
impl AdaptiveFeeVariables {
    ...
    pub fn update_reference(
        ...
        if current_timestamp < max_timestamp {
            return Err(anyhow::format_err!("InvalidTimestamp"));
        }
...
impl Oracle {
    ...
    pub fn deserialize(data: &[u8]) -> anyhow::Result<Self> {
        let whirlpool = Pubkey::new_from_array(
            data[8..40]
                .try_into()
                .context("Oracle::FailedToDeserialize::Whirlpool")?,
        );
        let trade_enable_timestamp = u64::from_le_bytes(
            data[40..48]
                .try_into()
                .context("Oracle::FailedToDeserialize::TradeEnableTimestamp")?,
        );
        let adaptive_fee_constants = AdaptiveFeeConstants {
            filter_period: u16::from_le_bytes(
                data[48..50]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::FilterPeriod")?,
            ),
            decay_period: u16::from_le_bytes(
                data[50..52]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::DecayPeriod")?,
            ),
            reduction_factor: u16::from_le_bytes(
                data[52..54]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::ReductionFactor")?,
            ),
            adaptive_fee_control_factor: u32::from_le_bytes(
                data[54..58]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::AdaptiveFeeControlFactor")?,
            ),
            max_volatility_accumulator: u32::from_le_bytes(
                data[58..62]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::MaxVolatilityAccumulator")?,
            ),
            tick_group_size: u16::from_le_bytes(
                data[62..64]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::TickGroupSize")?,
            ),
            major_swap_threshold_ticks: u16::from_le_bytes(
                data[64..66]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::MajorSwapThresholdTicks")?,
            ),
        };
        let adaptive_fee_variables = AdaptiveFeeVariables {
            last_reference_update_timestamp: u64::from_le_bytes(
                data[82..90]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::LastReferenceUpdateTimestamp")?,
            ),
            last_major_swap_timestamp: u64::from_le_bytes(
                data[90..98]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::LastMajorSwapTimestamp")?,
            ),
            volatility_reference: u32::from_le_bytes(
                data[98..102]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::VolatilityReference")?,
            ),
            tick_group_index_reference: i32::from_le_bytes(
                data[102..106]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::TickGroupIndexReference")?,
            ),
            volatility_accumulator: u32::from_le_bytes(
                data[106..110]
                    .try_into()
                    .context("Oracle::FailedToDeserialize::VolatilityAccumulator")?,
            ),
        };
    ...
    //     ) {
    //         return Err(anyhow::format_err!("InvalidAdaptiveFeeConstants"));
    //     }
...
pub struct OracleAccessor {
    // <'info> {
    pub oracle_account_info: Oracle, // AccountInfo<'info>,
    pub oracle_account_initialized: bool,
}

impl OracleAccessor {
    // pub fn new(whirlpool: &Account, oracle_account_info: AccountInfo<'info>) -> anyhow::Result<Self> {
    ...
    // pub fn is_trade_enabled(&self, current_timestamp: u64) -> anyhow::Result<bool> {
    //     if !self.oracle_account_initialized {
    //         return Ok(true);
    //     }

    //     // let oracle = self.load()?;
    //     Ok(self.oracle_account_info.trade_enable_timestamp <= current_timestamp)
    // }

    pub fn get_adaptive_fee_info(&self) -> anyhow::Result<Option<AdaptiveFeeInfo>> {
        if !self.oracle_account_initialized {
            return Ok(None);
        }
        ...
        Ok(Some(AdaptiveFeeInfo {
            constants: self.oracle_account_info.adaptive_fee_constants,
            variables: self.oracle_account_info.adaptive_fee_variables,
        }))
    }

    // pub fn update_adaptive_fee_variables(&self, adaptive_fee_info: &Option<AdaptiveFeeInfo>) -> anyhow::Result<()> {
    ...
    //         return Err(anyhow::format_err!("AccountOwnedByWrongProgram"));
    //     }

    //     let data = oracle_account_info.try_borrow_data()?;
    //     if data.len() < Oracle::discriminator().len() {
    //         return Err(anyhow::format_err!("AccountDiscriminatorNotFound"));
    //     }

    //     let disc_bytes = arrayref::array_ref![data, 0, 8];
    //     if disc_bytes != &Oracle::discriminator() {
    //         return Err(anyhow::format_err!("AccountDiscriminatorMismatch"));
    //     }
    ...
    // fn load(&self) -> anyhow::Result<Ref<'_, Oracle>> {
    ...
    // fn load_mut(&self) -> anyhow::Result<RefMut<'_, Oracle>> {
    //     // is_oracle_account_initialized already checked if the account is initialized
    ...
    //         return Err(anyhow::format_err!("AccountNotMutable"));
    //     }
