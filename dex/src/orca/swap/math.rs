use crate::orca::swap::u256_math::{mul_u256, U256Muldiv};
...
pub fn div_round_up_if(n: u128, d: u128, round_up: bool) -> anyhow::Result<u128> {
    if d == 0 {
        return Err(anyhow::format_err!("DivideByZero"));
    }
...
pub fn checked_mul_div_round_up(n0: u128, n1: u128, d: u128) -> anyhow::Result<u128> {
    checked_mul_div_round_up_if(n0, n1, d, true)
}

pub fn checked_mul_div_round_up_if(n0: u128, n1: u128, d: u128, round_up: bool) -> anyhow::Result<u128> {
    if d == 0 {
        return Err(anyhow::format_err!("DivideByZero"));
    }

    let p = n0.checked_mul(n1).ok_or(anyhow::format_err!("MulDivOverflow"))?;
    let n = p / d;

    Ok(if round_up && p % d > 0 { n + 1 } else { n })
}

pub fn checked_mul_div(n0: u128, n1: u128, d: u128) -> anyhow::Result<u128> {
    checked_mul_div_round_up_if(n0, n1, d, false)
}

// https://github.com/orca-so/whirlpools/blob/main/programs/whirlpool/src/math/liquidity_math.rs

// Adds a signed liquidity delta to a given integer liquidity amount.
// Errors on overflow or underflow.
pub fn add_liquidity_delta(liquidity: u128, delta: i128) -> anyhow::Result<u128> {
    if delta == 0 {
        return Ok(liquidity);
    }
    if delta > 0 {
        liquidity
            ...
            .ok_or(anyhow::format_err!("LiquidityOverflow"))
    } else {
        liquidity
            .checked_sub(delta.unsigned_abs())
            .ok_or(anyhow::format_err!("LiquidityUnderflow"))
    }
}

// Converts an unsigned liquidity amount to a signed liquidity delta
// pub fn convert_to_liquidity_delta(liquidity_amount: u128, positive: bool) -> anyhow::Result<i128> {
...
//         return Err(anyhow::format_err!("LiquidityTooHigh"));
//     }
//     Ok(if positive {
