use crate::orca::swap::{
    math::{div_round_up_if, div_round_up_if_u256, MAX_SQRT_PRICE_X64, MIN_SQRT_PRICE_X64, Q64_MASK, Q64_RESOLUTION},
...
pub fn try_get_amount_delta_a(
    ...
    let numerator = mul_u256(liquidity, sqrt_price_diff)
        ...
        .ok_or(anyhow::format_err!("MultiplicationOverflow"))?;
    ...
    match result {
        Ok(result) => {
            if result > u64::MAX as u128 {
                return Ok(AmountDeltaU64::ExceedsMax(anyhow::format_err!("TokenMaxExceeded")));
            }
...
pub fn try_get_amount_delta_b(
    ...
    if let Some(p) = n0.checked_mul(n1) {
        ...
        if should_round && result == u64::MAX {
            return Ok(AmountDeltaU64::ExceedsMax(anyhow::format_err!(
        ...
        Ok(AmountDeltaU64::ExceedsMax(anyhow::format_err!(
...
pub fn get_next_sqrt_price_from_a_round_up(
    ...
    if price < MIN_SQRT_PRICE_X64 {
        return Err(anyhow::format_err!("TokenMinSubceeded"));
    } else if price > MAX_SQRT_PRICE_X64 {
        return Err(anyhow::format_err!("TokenMaxExceeded"));
    }
...
pub fn get_next_sqrt_price_from_b_round_down(
    ...
    if amount_specified_is_input {
        ...
        sqrt_price
            .checked_add(delta)
            .ok_or(anyhow::format_err!("SqrtPriceOutOfBounds"))
    } else {
