...
fn require_gt(x: u128, y: u128) -> Result<()> {
    if x <= y {
        return Err(anyhow::format_err!("{} <= {}", x, y));
    }
    Ok(())
}

fn require_gte(x: u128, y: u128) -> Result<()> {
    if x < y {
        return Err(anyhow::format_err!("{} <= {}", x, y));
    }
    Ok(())
...
pub fn add_delta(x: u128, y: i128) -> Result<u128> {
    let z: u128;
    if y < 0 {
        //z = x - u128::try_from(-y).un_wrap();
        z = x
            .checked_sub(u128::try_from(-y).context("LiquiditySubValueErr")?)
            .ok_or_else(|| anyhow::format_err!("LiquiditySubValueErr"))?;
        let _ = require_gt(x, z); //, ErrorCode::LiquiditySubValueErr);
    } else {
        //z = x + u128::try_from(y).un_wrap();
        z = x
            .checked_add(u128::try_from(y).context("LiquidityAddValueErr")?)
            .ok_or_else(|| anyhow::format_err!("LiquidityAddValueErr"))?;
        let _ = require_gte(z, x); //, ErrorCode::LiquidityAddValueErr);
    }
...
pub fn get_liquidity_from_amount_0(
    ...
    let intermediate = U128::from(sqrt_ratio_a_x64)
        ...
        .ok_or_else(|| anyhow::anyhow!("RaydiumCLMMGetLiquidityFromAmount0_1"))?;

    Ok(U128::from(amount_0)
        .mul_div_floor(intermediate, U128::from(sqrt_ratio_b_x64 - sqrt_ratio_a_x64))
        .ok_or_else(|| anyhow::anyhow!("RaydiumCLMMGetLiquidityFromAmount0_2"))?
        .as_u128())
}
...
pub fn get_liquidity_from_amount_1(
    ...
    Ok(U128::from(amount_1)
        ...
        .ok_or_else(|| anyhow::anyhow!("RaydiumCLMMGetLiquidityFromAmount1_1"))?
        .as_u128())
}
    ...
    let result = if round_up {
        U256::div_rounding_up(
            numerator_1
                ...
                .ok_or_else(|| anyhow::anyhow!("RaydiumCLMMGetDeltaAmount0_1"))?,
            U256::from(sqrt_ratio_a_x64),
        )
    } else {
        numerator_1
            .mul_div_floor(numerator_2, U256::from(sqrt_ratio_b_x64))
            .ok_or_else(|| anyhow::anyhow!("RaydiumCLMMGetDeltaAmount0_2"))?
            / U256::from(sqrt_ratio_a_x64)
    };
    if result > U256::from(u64::MAX) {
        return Err(anyhow::format_err!("MaxTokenOverflow"));
    }
    Ok(result.as_u64())
}
...
pub fn get_delta_amount_1_unsigned(
    ...
    .ok_or(anyhow::anyhow!("RaydiumCLMMGetDeltaAmount1"))?;
    if result > U256::from(u64::MAX) {
        return Err(anyhow::format_err!("MaxTokenOverflow"));
    }
    Ok(result.as_u64())
...
pub fn get_delta_amount_0_signed(sqrt_ratio_a_x64: u128, sqrt_ratio_b_x64: u128, liquidity: i128) -> Result<u64> {
    if liquidity < 0 {
        get_delta_amount_0_unsigned(
            sqrt_ratio_a_x64,
            sqrt_ratio_b_x64,
            u128::try_from(-liquidity).map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount0_1"))?,
            false,
        )
    } else {
        get_delta_amount_0_unsigned(
            sqrt_ratio_a_x64,
            sqrt_ratio_b_x64,
            u128::try_from(liquidity).map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount0_2"))?,
            true,
        )
    }
}
...
pub fn get_delta_amount_1_signed(sqrt_ratio_a_x64: u128, sqrt_ratio_b_x64: u128, liquidity: i128) -> Result<u64> {
    if liquidity < 0 {
        get_delta_amount_1_unsigned(
            sqrt_ratio_a_x64,
            sqrt_ratio_b_x64,
            u128::try_from(-liquidity).map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount1_1"))?,
            false,
        )
    } else {
        get_delta_amount_1_unsigned(
            sqrt_ratio_a_x64,
            sqrt_ratio_b_x64,
            u128::try_from(liquidity).map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount1_2"))?,
            true,
        )
    }
}
...
pub fn get_delta_amounts_signed(
    ...
    if tick_current < tick_lower {
        ...
        .map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount1"))?;
        ...
        .map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount2"))?;
        amount_1 = get_delta_amount_1_signed(
            tick_math::get_sqrt_price_at_tick(tick_lower)?,
            sqrt_price_x64_current,
            liquidity_delta,
        )
        .map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount3"))?;
    } else {
        amount_1 = get_delta_amount_1_signed(
            tick_math::get_sqrt_price_at_tick(tick_lower)?,
            tick_math::get_sqrt_price_at_tick(tick_upper)?,
        ...
        .map_err(|_| anyhow::format_err!("RaydiumCLMMGetDeltaAmount4"))?;
    }
