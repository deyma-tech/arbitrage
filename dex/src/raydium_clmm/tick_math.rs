...
pub fn get_sqrt_price_at_tick(tick: i32) -> Result<u128, anyhow::Error> {
    ...
    if abs_tick > MAX_TICK as u32 {
        return Err(anyhow::Error::msg("tick is out of bounds"));
    }
...
pub fn get_tick_at_sqrt_price(sqrt_price_x64: u128) -> Result<i32, anyhow::Error> {
    ...
    Ok(if tick_low == tick_high {
        tick_low
    } else if get_sqrt_price_at_tick(tick_high).context("Overflow get_sqrt_price_at_tick")? <= sqrt_price_x64 {
        tick_high
    } else {
        tick_low
    })
