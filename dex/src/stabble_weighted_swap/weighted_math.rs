use crate::stabble_stable_swap::stable_math::{FixedComplement, FixedDiv, FixedMul, ONE};
...
pub fn calc_invariant(balances: &[u64], normalized_weights: &[u64]) -> Option<u64> {
    /**********************************************************************************************
    // invariant               _____                                                             //
...
pub fn calc_in_given_out(
    ...
    balance_in.mul_up(power.checked_sub(ONE)?)
}
...
pub fn calc_pool_token_out_given_exact_token_in(
    ...
    let amount_in_without_fee = if balance_ratio_with_fee > invariant_ratio_with_fees {
        let non_taxable_amount = if invariant_ratio_with_fees > ONE {
            balance.mul_down(invariant_ratio_with_fees.checked_sub(ONE)?)?
        } else {
            0
        };
...
pub fn calc_pool_token_out_given_exact_tokens_in(
    ...
    for i in 0..balances.len() {
        ...
        if balance_ratios_with_fee[i] > invariant_ratio_with_fees {
            ...
            let non_taxable_amount = if invariant_ratio_with_fees > ONE {
                balances[i].mul_down(invariant_ratio_with_fees.checked_sub(ONE)?)?
            } else {
                0
            };
...
pub fn calc_token_out_given_exact_pool_token_in(
    ...
    let balance_ratio = invariant_ratio.pow_up(ONE.div_down(normalized_weight)?)?;
