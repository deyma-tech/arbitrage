...
// use super::utils::{err, function, location};
// use super::utils::{TrackableError, TrackableResult};

pub const PRICE_LIQUIDITY_DENOMINATOR: u128 = 1__0000_0000__0000_0000__00u128;

#[decimal(24)]
//#[zero_copy]
#[derive(Default, std::fmt::Debug, PartialEq, Eq, PartialOrd, Ord, Copy, Clone)]
pub struct Price {
    pub v: u128,
}
...
impl Price {
    ...
    pub fn checked_big_div_values_up(nominator: U256, denominator: U256) -> anyhow::Result<Price> {
        Ok(Price::new(
            nominator
                .checked_mul(Self::one::<U256>())
                .ok_or_else(|| anyhow::format_err!("TrackableError::MUL"))?
                .checked_add(
                    denominator
                        .checked_sub(U256::from(1u32))
                        .ok_or_else(|| anyhow::format_err!("TrackableError::SUB"))?,
                )
                .ok_or_else(|| anyhow::format_err!("TrackableError::ADD"))?
                .checked_div(denominator)
                .ok_or_else(|| anyhow::format_err!("TrackableError::DIV"))?
                .try_into()
                .map_err(|_| anyhow::format_err!("TrackableError::cast::<Self>().as_str()"))?,
        ))
    }
}
