...
use crate::meteora_damm_v2::math::u128x128_math::mul_div_u256;

use super::math::{safe_math::SafeMath, u128x128_math::Rounding};
...
pub fn get_delta_amount_a_unsigned(
    ...
    if result > U256::from(u64::MAX) {
        return Err(anyhow::format_err!("MathOverflow"));
    }
    result.try_into().map_err(|_| anyhow::format_err!("TypeCastFailed"))
}
...
pub fn get_delta_amount_a_unsigned_unchecked(
    ...
    let result = mul_div_u256(numerator_1, numerator_2, denominator, round)
        .ok_or_else(|| anyhow::format_err!("MathOverflow"))?;
    Ok(result)
}
...
pub fn get_next_sqrt_price_from_input(
    ...
    if sqrt_price == 0 {
        return Err(format_err!("ZeroSqrtPrice"));
    }
    if liquidity == 0 {
        return Err(format_err!("ZeroLiquidity"));
    }
...
///  √P' = √P * L / (L + Δx * √P)
///
pub fn get_next_sqrt_price_from_amount_a_rounding_up(sqrt_price: u128, liquidity: u128, amount: u64) -> Result<u128> {
    if amount == 0 {
...
/// * `√P' = √P + Δy / L`
///
pub fn get_next_sqrt_price_from_amount_b_rounding_down(sqrt_price: u128, liquidity: u128, amount: u64) -> Result<u128> {
