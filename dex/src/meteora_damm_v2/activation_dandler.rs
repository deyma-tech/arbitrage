...
pub fn get_current_point(activation_type: u8, slot: u64) -> Result<u64> {
    let activation_type =
        ActivationType::try_from(activation_type).map_err(|_| anyhow::format_err!("InvalidActivationType"))?;
    let current_point = match activation_type {
        ActivationType::Slot => slot,
        ActivationType::Timestamp => now::as_secs(),
    };
    Ok(current_point)
}
...
//             .map_err(|_| anyhow::format_err!("InvalidActivationType"))?;
//         let current_point = match activation_type {
//             ActivationType::Slot => slot,
//             ActivationType::Timestamp => now::as_secs(),
//         };
...
//     pub fn get_current_point_and_max_vesting_duration(activation_type: u8, slot: u64) -> Result<(u64, u64)> {
//         let activation_type = ActivationType::try_from(activation_type)
//             .map_err(|_| anyhow::format_err!("InvalidActivationType"))?;
//         let (curr_point, max_vesting_duration) = match activation_type {
//             ActivationType::Slot => (slot, MAX_VESTING_SLOT_DURATION),
...
//     pub fn get_current_point_and_buffer_duration(activation_type: u8, slot: u64) -> Result<(u64, u64)> {
//         let activation_type = ActivationType::try_from(activation_type)
//             .map_err(|_| anyhow::format_err!("InvalidActivationType"))?;
//         let (curr_point, buffer_duration) = match activation_type {
//             ActivationType::Slot => (slot, SLOT_BUFFER),
//             ActivationType::Timestamp => (now::as_secs(), TIME_BUFFER),
//         };
//         Ok((curr_point, buffer_duration))
//     }

//     pub fn get_max_activation_point(activation_type: u8, slot: u64) -> Result<u64> {
//         let activation_type = ActivationType::try_from(activation_type)
//             .map_err(|_| anyhow::format_err!("InvalidActivationType"))?;
//         let (curr_point, max_activation_duration) = match activation_type {
//             ActivationType::Slot => (slot, MAX_ACTIVATION_SLOT_DURATION),
//             ActivationType::Timestamp => (
//                 now::as_secs(),
...
//         //     anyhow::format_err!("PoolDisabled")
//         // );
//         if self.curr_point < self.activation_point {
//             return Err(anyhow::format_err!("PoolDisabled"));
//         }
//         Ok(())
//     }
...
//         //     anyhow::format_err!("UnableToModifyActivationPoint")
//         // );
//         if new_activation_point <= nearest_new_activation_point ||
//             self.activation_point <= self.curr_point {
//             return Err(anyhow::format_err!("UnableToModifyActivationPoint"));
//         }
...
//             //     anyhow::format_err!("UnableToModifyActivationPoint")
//             // );
//             if self.curr_point >= self.get_pre_activation_start_point()? {
//                 return Err(anyhow::format_err!("UnableToModifyActivationPoint"));
//             }
...
//             //     anyhow::format_err!("UnableToModifyActivationPoint")
//             // );
//             if self.curr_point >= buffered_new_pre_activation_start_point {
//                 return Err(anyhow::format_err!("UnableToModifyActivationPoint"));
//             }
//         }
