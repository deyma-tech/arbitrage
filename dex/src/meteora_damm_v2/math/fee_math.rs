use crate::meteora_damm_v2::constants::{BASIS_POINT_MAX, ONE_Q64};
...
pub fn get_fee_in_period(cliff_fee_numerator: u64, reduction_factor: u64, passed_period: u16) -> anyhow::Result<u64> {
    ...
    let result = pow(base, passed_period.into()).ok_or_else(|| anyhow::format_err!("MathOverflow"))?;

    let (fee, _) = result
        .safe_mul(cliff_fee_numerator.into())?
        .overflowing_shr(SCALE_OFFSET);

    let fee_numerator = u64::try_from(fee).map_err(|_| anyhow::format_err!("TypeCastFailed"))?;
    Ok(fee_numerator)
}
