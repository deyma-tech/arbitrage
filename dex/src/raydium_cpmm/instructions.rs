...
impl RaydiumCpmmProgramIx {
    pub fn deserialize(buf: &[u8]) -> std::io::Result<Self> {
        ...
        match maybe_discm {
            ...
            // UPDATE_POOL_STATUS_IX_DISCM => Ok(Self::UpdatePoolStatus(UpdatePoolStatusIxArgs::deserialize(
            //     &mut reader,
            // )?)),
...
//             return Err(std::io::Error::other(format!(
//                 "discm does not match. Expected: {UPDATE_POOL_STATUS_IX_DISCM:?}. Received: {maybe_discm:?}"
//             )));
//         }
