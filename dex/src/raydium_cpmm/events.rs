    ...
    pub fn deserialize(buf: &mut &[u8]) -> std::io::Result<Self> {
        ...
        if maybe_discm != LP_CHANGE_EVENT_EVENT_DISCM {
            return Err(std::io::Error::other(format!(
                "discm does not match. Expected: {LP_CHANGE_EVENT_EVENT_DISCM:?}. Received: {maybe_discm:?}"
            )));
        }
...
impl SwapEventEvent {
    pub fn deserialize(buf: &mut &[u8]) -> std::io::Result<Self> {
        ...
        if maybe_discm != SWAP_EVENT_EVENT_DISCM {
            return Err(std::io::Error::other(format!(
                "discm does not match. Expected: {SWAP_EVENT_EVENT_DISCM:?}. Received: {maybe_discm:?}"
            )));
        }
